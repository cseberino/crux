...
"""
        ...
        def test_produce(self):
                ...
                output = crux.produce(["True"])
                answer = True
                self.assertEqual(output, answer)

                output = crux.produce(["False"])
                answer = False
                self.assertEqual(output, answer)

                output = crux.produce(["42"])
                answer = 42
                self.assertEqual(output, answer)

                output = crux.produce(["-789"])
                answer = -789
                self.assertEqual(output, answer)

                output = crux.produce(['"hello"'])
                answer = "hello"
                self.assertEqual(output, answer)

                output = crux.produce(["abc"])
                answer = ("abc",)
                self.assertEqual(output, answer)
...
'''
        ...
        def test_evaluation_model(self):
                program = \
'''
# In the environment, f and g will be associated with the results of evaluting
...
test_suite = unittest.makeSuite(TestEmulator)
